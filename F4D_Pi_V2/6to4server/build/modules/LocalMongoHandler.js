...
const Device_Owner = Models_1.Conf.Device_Owner.Owner;
...
const mac_add = Models_1.Conf.BlackBox.bucket;
const dbPromise = client.connect().then(() => client.db(mac_add));
async function updateSensorLastSeen(ipv6, timestamp) {
    ...
    for (const collection of collections) {
        ...
        if (collection.name === "exp_0_BOOT") {
            ...
            if (sensor) {
                await db.collection(collection.name).updateOne({ 'SensorData.LLA': ipv6 }, { $set: { 'SensorData.LastSeen': timestamp } });
            }
        }
...
async function PackageToSendMongo(pkg, timeStamp, buffer, exp_name) {
    const db = await dbPromise;
    ...
    await targetCollection.updateMany({ "MetaData.LLA": additionalExperimentData.SensorData.LLA }, {
        $set: {
...
async function getClientSensors() {
    ...
    for (const sensor of bootSensors) {
        const sensorExists = activeSensors.some((activeSensor) => activeSensor.SensorData.LLA === sensor.SensorData.LLA);
        if (!sensorExists) {
...
async function updateActiveSensorsInfo() {
    const db = await dbPromise;
    const collections = await db.listCollections().toArray();
    const activeSensors = [];
...
async function removeSensorFromBoot(LLA) {
    const db = await dbPromise;
    const collection = db.collection('exp_0_BOOT');
...
async function getExpNameByLLA(LLA) {
    const db = await dbPromise;
    const collections = await db.listCollections().toArray();
    const found = false;
    for (const collection of collections) {
...
async function deleteFromBoot(expName) {
    const db = await dbPromise;
    const collection = db.collection('exp_0_BOOT');
    try {
...
async function isExistsByIpv6(ipv6, collectionName) {
    const db = await dbPromise;
...
async function addSensorCoordinates(sensor, coordinates) {
    ...
    for (const collection of collections) { // of  is used to iterate over the values in an iterable, like an array
        const SensorInCollection = await db.collection(collection.name).findOne({ 'SensorData.LLA': sensor.SensorData.LLA, 'SensorData.isActive': true });
        // console.log("the sensor is:",sensor.SensorData.LLA)
        if (SensorInCollection) {
            collectionName = collection.name;
            await db.collection(collectionName).updateOne({ "SensorData.LLA": sensor.SensorData.LLA }, { $set: { 'SensorData.coordinates': coordinates } } // Update operation
            );
...
async function addSensorData(sensor, collectionName) {
    const db = await dbPromise;
    const collection = db.collection(collectionName);
    try {
        ...
        const res = await collection.updateOne({ "SensorData.LLA": sensor.SensorData.LLA }, { $set: updateData }, // Update the fields
...
async function updateLabel(dataArray) {
    const db = await dbPromise;
        ...
        if (sensor.SensorData.LLA) {
            ...
            for (const collection of collections) {
                ...
                if (activeSensor) {
                    ...
                    await collection.updateMany({ "SensorData.LLA": sensor.SensorData.LLA, "SensorData.isActive": true }, {
                        $set: {
                            "SensorData.Label": sensor.SensorData.Label,
                            "SensorData.LabelOptions": sensor.SensorData.LabelOptions,
                        }
            ...
            if (!updated) {
                await bootCollection.updateMany({ "SensorData.LLA": sensor.SensorData.LLA }, {
                    $set: {
                        "SensorData.Label": sensor.SensorData.Label,
                        "SensorData.LabelOptions": sensor.SensorData.LabelOptions,
                    }
...
async function createNewCollection(expName) {
    const db = await dbPromise;
...
async function findLastCollectionId() {
    const db = await dbPromise;
    const collections = await db.listCollections().toArray();
 ...
 * @param ipv6
 */
async function getSensorInfoByIpv6(ipv6) {
    const db = await dbPromise;
    const collections = await db.listCollections().toArray();
    let SensorData = {};
    let SensorAlerted = {};
 ...
 * ```
 */
async function updateSensorAlertedMail(expName, alertedmail) {
    ...
    for (const collection of collections) {
        ...
        if (alertedmail === '') {
            ...
            const updateResult = await db.collection(collection.name).updateMany({ 'ExperimentData.Exp_name': expName, 'SensorData.isActive': true }, { $set: { "Alerts.Email": '' } });
            ...
            const sensors = await db.collection(collection.name).find({ 'ExperimentData.Exp_name': expName, 'SensorData.isActive': true }).toArray();
            for (const sensor of sensors) {
                // Check if email already exists in the alerts field
                const emails = sensor.Alerts?.Email?.split(',').map((email) => email.trim()) || [];
                if (!emails.includes(alertedmail)) {
                    let updatedEmail = sensor.Alerts?.Email ? `${sensor.Alerts.Email}, ${alertedmail}` : alertedmail;
                    const updateResult = await db.collection(collection.name).updateOne({ _id: sensor._id }, { $set: { "Alerts.Email": updatedEmail } });
 ...
 *
 */
async function MongoSwitchSensorsList(ipv6ofCurrentSensor, ipv6ofNewSensor, sensorList) {
    console.log('/processing Switch Sensors List');
    const db = await dbPromise; // Connect to mongoDB
    // get first and second sensors as string
    const currentSensor = sensorList.find((sensor) => sensor.SensorData.LLA === ipv6ofCurrentSensor);
    const newSensor = sensorList.find((sensor) => sensor.SensorData.LLA === ipv6ofNewSensor);
    // find the collection where current sensor is active
    const collections = await db.listCollections().toArray();
    let collectionName = '';
    for (const collection of collections) {
        const sensorInCollection = await db.collection(collection.name).findOne({ 'SensorData.LLA': ipv6ofCurrentSensor, 'SensorData.isActive': true });
        if (sensorInCollection) {
            collectionName = collection.name;
            break;
        }
    ...
    if (documentCurrent) {
        ...
        const updatedResult = await db.collection(collectionName).insertOne(fieldsToUpdate);
        ...
        const invalidateCurrentSensor = await db.collection(collectionName).updateOne(query_1, // find the current sensor
        {
            $set: {
...
//   }
// }
async function UpdateByCSV(dataArray) {
    const db = await dbPromise;
    const collection = db.collection("exp_0_BOOT");
 ...
 * @author Nir
 */
async function getExperimentsInfo() {
    const db = await dbPromise; // Connect to mongoDB
    const collections = await db.listCollections().toArray(); // Retrieve all collections
    ...
    for (const collection of collections) {
        ...
        if (!seenExperiments.has(expName)) {
            ...
            experimentInfo.add({
                ...
                sensor_info: data.reduce((acc, curr) => {
