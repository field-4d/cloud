...
function App() {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${protocolPrefix}://${window.location.hostname}:8080`);
  ...
  function sendEvent(e) {
    if (isConnected && webSocket && e !== lastEvent) {
      try {
      ...
      console.warn('WebSocket is not connected. Cannot send event:', e);
    }
  }
