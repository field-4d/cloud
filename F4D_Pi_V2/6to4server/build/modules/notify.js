...
exports.sendSerialPortErrorAlert = exports.DeadManAlerts = exports.sendAccumulatedAlerts = exports.alerter_2 = exports.alerter = exports.batteryAlerter = void 0;
...
function batteryAlerter(obj) {
    const activeSensorEntry = LocalMongoHandler_1.activeSensorsInfo.find(info => info.LLA === obj.ipv6);
...
const sensorSendCounts = new Map();
...
async function alerter_2(obj) {
    const { SensorData: SensorData, SensorAlerted: SensorAlerted, SensorExperiment: SensorExperiment } = await (0, LocalMongoHandler_1.getSensorInfoByIpv6)(obj.ipv6);
        ...
        if (message) {
            ...
            if (filteredMessage) {
                ...
                const headers = "Current Time,Experiment Name,Location,IPv6 Address,Type,Value,Range"
                    ...
                    .join('');
                const rows = filteredMessage.split('\n').map(line => `<tr>${line.split(',').map(cell => `<td>${cell.trim()}</td>`).join('')}</tr>`).join('');
...
async function DeadManAlerts() {
    // Update the active sensors info before processing alerts
    await (0, LocalMongoHandler_1.updateActiveSensorsInfo)();
    const currentTime = new Date();
    ...
    const alertsByExperiment = {};
    LocalMongoHandler_1.activeSensorsInfo.forEach((sensor) => {
 ...
 */
function sendSerialPortErrorAlert(err) {
    const BUCKET_NAME = process.env.BUCKET_NAME || 'UNKNOWN_MAC';
    const ALERT_EMAILS = 'menachem.moshelion@mail.huji.ac.il, nir.averbuch@mail.huji.ac.il';
...
exports.sendSerialPortErrorAlert = sendSerialPortErrorAlert;
