...
const OUTPUT_DIRECTORY = path.resolve('docs/benchmarks/phase4c2_local_integration');
...
async function runOnce(page: import('@playwright/test').Page, workloadId: WorkloadId) {
  ...
  try {
    ...
    expect(result.endpoint).toBe('http://localhost:3001/api/v2/fetch-data-page');
...
test('measure integrated local v2 E2E through plot in headed Chromium', async ({ page }) => {
  ...
  console.log('PHASE4C2_BROWSER_WARMUP C0-A');
  ...
  for (const workloadId of Object.keys(REPETITIONS) as WorkloadId[]) {
    for (let repetition = 1; repetition <= REPETITIONS[workloadId]; repetition += 1) {
      console.log(`PHASE4C2_BROWSER_MEASURED ${workloadId} repetition=${repetition}`);
      const result = await runOnce(page, workloadId);
      results[workloadId].push({ repetition, ...result });
      await mkdir(OUTPUT_DIRECTORY, { recursive: true });
      await writeFile(OUTPUT_PATH, `${JSON.stringify({
        benchmark: 'FIELD4D_PHASE4C2_LOCAL_BACKEND_INTEGRATION',
