...
def main():
    """
    Run batching scenarios for Scenario 6 only (40,320 data points).
    Quick test to validate batching approach with comprehensive logging.
    """
    print("=== ANOVA API Batching Performance Test - Scenario 6 Only ===")
    print("Scenario 6: 15-min, 3 groups, 10 reps, 14 days (40,320 points, 37.87s original)")
    
    # Generate unique test run ID
    test_run_id = f"scenario6_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
    print(f"Test Run ID: {test_run_id}")
    
    test_results = []
    
    # Generate Scenario 6 data
    print("\nGenerating Scenario 6 dataset...")
    scenario6_data = generate_data("2025-06-01", 14, ["A", "B", "C"], 10, freq_minutes=15)
    print(f"Generated {len(scenario6_data)} data points")
    
    # Test 1: Sequential by Days (7 days per batch)
    test1_id = f"{test_run_id}_test1_days_seq"
    result1 = run_batched_test(
        "Scenario 6 - Sequential by Days (7 days/batch)",
        scenario6_data, "days", 7, test_id=test1_id
    )
    test_results.append(result1)
    
    # Test 2: Sequential by Size (10K points per batch)
    test2_id = f"{test_run_id}_test2_size_seq"
    result2 = run_batched_test(
        "Scenario 6 - Sequential by Size (10K points/batch)",
        scenario6_data, "size", 10000, test_id=test2_id
    )
    test_results.append(result2)
    
    # Test 3: Concurrent by Size (10K points, 1 worker)
    test3_id = f"{test_run_id}_test3_size_1worker"
    result3 = run_concurrent_batched_test(
        "Scenario 6 - Concurrent by Size (10K points/batch, 1 worker)",
        scenario6_data, "size", 10000, max_workers=1, test_id=test3_id
    )
    test_results.append(result3)
    
    # Test 4: Concurrent by Size (10K points, 2 workers)
    test4_id = f"{test_run_id}_test4_size_2workers"
    result4 = run_concurrent_batched_test(
        "Scenario 6 - Concurrent by Size (10K points/batch, 2 workers)",
        scenario6_data, "size", 10000, max_workers=2, test_id=test4_id
    )
    ...
    with open(os.path.join(datasets_dir, "scenario6_data.json"), "w") as f:
        json.dump(scenario6_data, f, indent=2)
    print(f"Saved Scenario 6 data: {len(scenario6_data)} points")
    ...
    df['Original_Scenario'] = '6'
    df['Original_Time_Seconds'] = 37.87
    df['Original_Time_Minutes'] = 37.87 / 60
    df['Speedup_Factor'] = 37.87 / df['test_duration_seconds']
    ...
    # Save to CSV
    csv_path = os.path.join(OUTPUT_DIR, "scenario6_batching_results.csv")
    ...
    run_summary = {
        ...
        "total_data_points": len(scenario6_data),
    ...
    # Print summary
    print("\n=== Scenario 6 Batching Performance Summary ===")
    ...
    print(f"Estimated time: 1-4 minutes (actual: {df['Total_Time_Minutes'].sum():.2f} minutes)")
