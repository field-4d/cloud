...
# Load environment variables from auth/.env or ENV_FILE_PATH (Cloud Run mount)
# ENV_FILE_PATH overrides default path when set (e.g. /secrets/.env)
_env_file_override = os.getenv("ENV_FILE_PATH")
env_path = Path(_env_file_override) if _env_file_override else Path(__file__).parent / ".env"
...
async def get_client():
    ...
    # Priority 1: Load .env file (ENV_FILE_PATH override for Cloud Run, else auth/.env)
    _env_override = os.getenv("ENV_FILE_PATH")
    env_path = Path(_env_override) if _env_override else Path(__file__).parent / ".env"
